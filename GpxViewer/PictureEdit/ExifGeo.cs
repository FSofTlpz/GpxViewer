...
namespace GpxViewer.PictureEdit {
   class ExifGeo {
      ...
      public bool SetDateTimeOriginal(DateTime dateTime) {
         bool set = false;
         ExifProperty? prop = data.Properties.FirstOrDefault(p => p.Tag == ExifTag.DateTimeOriginal);
         if (dateTime == DateTime.MinValue) {
            ...
            if (prop == null) {
               data.Properties.Set(ExifTag.DateTimeOriginal, dateTime);
               set = true;
            } else {
               prop.Value = dateTime;
               set = true;
      ...
      public string GetUserComment() {
         ...
         string? tmp = prop?.Value as string;
      ...
      public bool SetUserComment(string txt) {
         ...
         if (string.IsNullOrEmpty(txt)) {
            ...
            // NUR ExifLibrary.JPEGFile kennt ByteOrder!
            if (data is JPEGFile &&
                (data as JPEGFile).ByteOrder == BitConverterEx.ByteOrder.BigEndian) {
               // BitConverterEx.ByteOrder.BigEndian oder BitConverterEx.ByteOrder.LittleEndian
               // BitConverterEx.ByteOrder byteOrder = (data as ExifLibrary.JPEGFile).ByteOrder;
      ...
      /// <summary>
      /// speichert die Datei mit den akt. Daten (erst als temporäre Datei, danach umbenannt)
      /// </summary>
      ...
      double readExifLonOrLat(bool vislon) {
         ...
         if (prop != null) {
            v = (double)(prop as GPSLatitudeLongitude).Degrees +
                (double)(prop as GPSLatitudeLongitude).Minutes / 60.0 +
                (double)(prop as GPSLatitudeLongitude).Seconds / 3600.0;    // Grad, Minuten, Sekunden
