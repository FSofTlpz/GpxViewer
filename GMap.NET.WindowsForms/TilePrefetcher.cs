    ...
    public partial class TilePrefetcher : Form
    {
        ...
        void worker_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            ...
            if (Overlay != null)
            {
                ...
                if (l.HasValue)
                {
                    var px = Overlay.Control.Map_Provider.Projection.FromTileXYToPixel(l.Value);
                    var p = Overlay.Control.Map_Provider.Projection.FromPixelToLatLng(px, _zoom);

                    double r1 = Overlay.Control.Map_Provider.Projection.GetGroundResolution(_zoom, p.Lat);
                    double r2 = Overlay.Control.Map_Provider.Projection.GetGroundResolution((int)Overlay.Control.Map_Zoom,
                        p.Lat);
                    double sizeDiff = r2 / r1;

                    var m = new GMapMarkerTile(p,
                        (int)(Overlay.Control.Map_Provider.Projection.TileSize.Width / sizeDiff));
