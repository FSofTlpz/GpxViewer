...
namespace FSofTUtils.Geography {
   ...
   /// <summary>
   /// versucht, eine KML/KMZ-Datei einzulesen
   /// <para>Ein Problem sind die offensichtlich unterschiedlichen Möglichkeiten mit denen Waypoints und Tracks definiert werden können. Deshalb werden
   /// verschiedene Varianten ausprobiert (bei Bedarf erweitern!).</para>
   /// </summary>
   public class KmlReader {
      ...
      const string PRIVATENAMESPACE = "x";

      static class XPathHelper {

         static char[] spacesplitted = new char[] { ' ' };
         static char[] commasplitted = new char[] { ',' };

         /// <summary>
         /// Anzahl der Document-Childs unter diesem XPath
         /// </summary>
         /// <param name="kml"></param>
         /// <param name="xpath"></param>
         /// <returns></returns>
         static public int GetDocumentkCount4XPath(SimpleXmlDocument2 kml, string xpath) {
            return kml.NodeCount(xpathWithNamespace(xpath + "/Document"));
         }

         /// <summary>
         /// Anzahl der Folder-Childs unter diesem XPath
         /// </summary>
         /// <param name="kml"></param>
         /// <param name="xpath"></param>
         /// <returns></returns>
         static public int GetFolderCount4XPath(SimpleXmlDocument2 kml, string xpath) {
            return kml.NodeCount(xpathWithNamespace(xpath + "/Folder"));
         }

         /// <summary>
         /// Anzahl der Placemark-Childs unter diesem XPath
         /// </summary>
         /// <param name="kml"></param>
         /// <param name="xpath"></param>
         /// <returns></returns>
         static public int GetPlacemarkCount4XPath(SimpleXmlDocument2 kml, string xpath) {
            return kml.NodeCount(xpathWithNamespace(xpath + "/Placemark"));
         }


         /// <summary>
         /// Versucht, für den XPath zu einem Placemark verschiedene Infos zu ermitteln
         /// </summary>
         /// <param name="kml"></param>
         ...
         static public void GetInfos4PlacemarkXPath(SimpleXmlDocument2 kml,
                                                    string xpath2placemark,
                                                    out string name,
            ...
            name = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/name"), "");

            description = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/description"), "");
            if (description == "")
               description = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/desc"), "");
            if (description == "")
               description = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/ExtendedData/SchemaData/SimpleData[@name=\"desc\"]"), "");

            comment = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/comment"), "");
            if (comment == "")
               comment = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/cmt"), "");
            if (comment == "")
               comment = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/ExtendedData/SchemaData/SimpleData[@name=\"cmt\"]"), "");

            symbol = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/symbol"), "");
            if (symbol == "")
               symbol = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/sym"), "");
            if (symbol == "")
               symbol = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/ExtendedData/SchemaData/SimpleData[@name=\"sym\"]"), "");

            elevation = PoorGpx.BaseElement.NOTVALID_DOUBLE;
            string ele = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/ExtendedData/SchemaData/SimpleData[@name=\"ele\"]"), "");
            if (!string.IsNullOrEmpty(ele.ToString()))
               elevation = getDouble(ele);

            time = getDateTime(kml.ReadValue(xpathWithNamespace(xpath2placemark + "/TimeStamp/when"), ""));
            if (time == PoorGpx.BaseElement.NOTVALID_TIME)
               time = getDateTime(kml.ReadValue(xpathWithNamespace(xpath2placemark + "/ExtendedData/SchemaData/SimpleData[@name=\"time\"]"), ""));

            timestart = getDateTime(kml.ReadValue(xpathWithNamespace(xpath2placemark + "/TimeSpan/begin"), ""));

            timeend = getDateTime(kml.ReadValue(xpathWithNamespace(xpath2placemark + "/TimeSpan/end"), ""));
         }

         /// <summary>
         /// Versucht, für den XPath zu einem Placemark den (einzelnen) Punkt zu ermitteln (dann ist es KEINE Linie)
         /// </summary>
         /// <param name="kml"></param>
         /// <param name="xpath2placemark"></param>
         /// <returns></returns>
         ...
         /// <summary>
         /// Versucht, für den XPath zu einem Placemark alle Punktlisten (Segmente) zu ermitteln
         /// </summary>
         /// <param name="kml"></param>
         /// <param name="xpath2placemark"></param>
         /// <returns></returns>
         ...
         static PoorGpx.ListTS<PoorGpx.GpxTrackPoint>? getPoints4Placemark(SimpleXmlDocument2 kml, string xpath2placemark, int segmentidx) {
            ...
            if (coordinates == "")
               coordinates = kml.ReadValue(xpathWithNamespace(xpath2placemark + "/MultiGeometry/LineString[" + (segmentidx + 1) + "]/coordinates"), "");
            ...
            if (coords != null)
               ...
               coords = kml.ReadString(xpathWithNamespace(xpath2placemark) + "/gx:MultiTrack/gx:Track[" + (segmentidx + 1) + "]/gx:coord");
               times = kml.ReadString(xpathWithNamespace(xpath2placemark) + "/gx:MultiTrack/gx:Track[" + (segmentidx + 1) + "]" + xpathWithNamespace("/when"));
            }
...
#pragma warning restore CS8604 // Mögliches Nullverweisargument.
         ...
         static PoorGpx.GpxTrackPoint? getPointFromText(string ptstring, char[] splitchars, string? timestring) {
            ...
            switch (data.Length) {
               case 2:
                  return new PoorGpx.GpxTrackPoint(getDouble(data[0]),
                                                   getDouble(data[1]),
                                                   PoorGpx.BaseElement.NOTVALID_DOUBLE,
         ...
         static public Color GetColor4PlacemarkXPath(SimpleXmlDocument2 kml, string xpath2document, string xpath2placemark) {
            ...
            if (col == Color.Empty) {
               ...
               if (stylemapname != "") {
                  if (stylemapname[0] == '#')
                     stylemapname = stylemapname.Substring(1);
                  string stylename = getColorStyle4NormalStylemapInDocumentXPath(kml, xpath2document, stylemapname);
                  if (stylename != "") {
                     if (stylename[0] == '#')
                        stylename = stylename.Substring(1);
                     return getColor4LinestyleInDocument(kml, xpath2document, stylename);
                  }
               }
            }
            return col;
         ...
         /// <summary>
         /// liefert, wenn vorhanden den Namen der Stylemap (mit führendem '#')
         /// </summary>
         /// <param name="kml"></param>
         /// <param name="xpath2placemark"></param>
         /// <returns></returns>
            ...
            if (pairkeys != null)
               for (int i = 0; i < pairkeys.Length; i++) {
                  if (pairkeys[i] == "normal")
                     return kml.ReadValue(xpathWithNamespace(xpath2document + "/StyleMap") + "[@id=\"" + stylemapname + "\"]" + xpathWithNamespace("/Pair[" + (i + 1) + "]/styleUrl"), "");
               }
            return "";
         }
         ...
         static Color hexToColor(string hexString) {
            ...
            if (hexString.Length == 6)
               return Color.FromArgb(255,
                                     getInt4Hex(hexString.Substring(0, 2)),
                                     getInt4Hex(hexString.Substring(2, 2)),
                                     getInt4Hex(hexString.Substring(4, 2))
                                    );
            else if (hexString.Length == 8)
               return Color.FromArgb(getInt4Hex(hexString.Substring(0, 2)),
                                     getInt4Hex(hexString.Substring(2, 2)),
                                     getInt4Hex(hexString.Substring(4, 2)),
      ...
      void read(SimpleXmlDocument2 kml, PoorGpx.GpxAll gpx, List<Color> colors, string xpath, string lastdocumentxpath) {
         ...
         int folders = XPathHelper.GetFolderCount4XPath(kml, xpath);
         int placemarks = XPathHelper.GetPlacemarkCount4XPath(kml, xpath);

         for (int i = 0; i < placemarks; i++) {
            string placemarkpath = xpath + "/Placemark[" + (i + 1) + "]";

            XPathHelper.GetInfos4PlacemarkXPath(kml, placemarkpath,
                                                out string name,
                                                out string description,
                                                out string comment,
                                                out string symbol,
            ...
            if (pt != null) {          // Placemark ist ein Waypoint
               ...
               PoorGpx.GpxWaypoint gpxWaypoint = new PoorGpx.GpxWaypoint(pt.Lon, pt.Lat, pt.Elevation, pt.Time) {
                  ...
                  Comment = comment,
                  Symbol = symbol,
                  Time = pt.Time != PoorGpx.BaseElement.NOTVALID_TIME ? pt.Time :
                         time != PoorGpx.BaseElement.NOTVALID_TIME ? time :
                         timestart != PoorGpx.BaseElement.NOTVALID_TIME ? timestart :
                         timeend != PoorGpx.BaseElement.NOTVALID_TIME ? timeend : PoorGpx.BaseElement.NOTVALID_TIME,
               };
               gpx.Waypoints.Add(gpxWaypoint);
               ...
               if (lstlst != null && lstlst.Count > 0) { // Track mmit min. 1 Segment
                  ...
                  for (int s = 0; s < lstlst.Count; s++) {
                     track.Segments.Add(new PoorGpx.GpxTrackSegment() {
                        Points = lstlst[s],
                     });
                  }
                  gpx.Tracks.Add(track);
                  colors.Add(XPathHelper.GetColor4PlacemarkXPath(kml, lastdocumentxpath, placemarkpath));
               }
         ...
         for (int i = 0; i < documents; i++) {
            string xpath2document = xpath + "/Document[" + (i + 1) + "]";
            read(kml, gpx, colors, xpath2document, xpath2document);
         }

         for (int i = 0; i < folders; i++)
            read(kml, gpx, colors, xpath + "/Folder[" + (i + 1) + "]", lastdocumentxpath);
      }

      /// <summary>
      /// liest die KMZ- oder KML-Datei ein
      /// </summary>
      ...
      /*
 		verwendbare Farbdef. für Tracks:
			/kml/Document/Style/@id[NAME]/LineStyle/color		   z.B. ff073ee7 /vermutlich ARGB

      u.a.

         * /Placemark[IDX]/styleUrl                            #stylemap_id1
         ->
         * /Document/StyleMap/@id=[STYLEMAPNAME]/Pair[IDX]/key == normal
         * /Document/StyleMap/@id=[STYLEMAPNAME]/Pair[IDX]/styleUrl
         ->
         * /Document/Style/@id=[STYLENAME]/LineStyle/color


		mögliche Trackdef.:
			/kml/Document/Placemark[IDX]/name
			/kml/Document/Placemark[IDX]/LineString/coordinates	Lon/Lat-Paare oder Lon/Lat/Ele-Triples (z.B. space-separated "10.90831029,50.38719123999999,510.69531")
			/kml/Document/Placemark[IDX]/description
			/kml/Document/Placemark[IDX]/styleUrl						Name des Stil (Farbe)
			/kml/Document/Placemark[IDX]/TimeSpan/begin				unnötig (z.B. "2010-12-20T03:00:00Z")
			/kml/Document/Placemark[IDX]/TimeSpan/end					unnötig
			/kml/Document/Placemark[IDX]/TimeStamp/when				unnötig

		mögliche Waypointdef.:
			/kml/Document/Placemark[IDX]/name
			/kml/Document/Placemark[IDX]/Point/coordinates			Lon/Lat-Paar
			/kml/Document/Placemark[IDX]/description
			/kml/Document/Placemark[IDX]/TimeSpan/begin
			/kml/Document/Placemark[IDX]/TimeSpan/end
			/kml/Document/Placemark[IDX]/TimeStamp/when


		mögliche Trackdef.:
			/kml/Document/Placemark[IDX]/name
			/kml/Document/Placemark[IDX]/gx:Track/when				Zeitpunktliste (!) (z.B. "2010-11-14T10:38:37Z")
			/kml/Document/Placemark[IDX]/gx:Track/gx:coord			Lon/Lat/Ele-Triple-Liste (!) (z.B. "13.938957 51.077421 280.8")


		mit Foldern:
			/kml/Document/Folder[IDX]/name

		mögliche Trackdef.:
			/kml/Document/Folder/Placemark[IDX]/name
			/kml/Document/Folder/Placemark[IDX]/MultiGeometry/LineString/coordinates					Lon/Lat/Ele-Triples oder Lon/Lat-Paare (z.B. space-separated "12.6433829404414,47.7585958968848")
			/kml/Document/Folder/Placemark[IDX]/Style/LineStyle/color										z.B. cc0055ff

         auch geschachtelt (!) mit weiteren Document/Folder

		mögliche Waypointdef.:
			/kml/Document/Folder/Placemark[IDX]/name
			/kml/Document/Folder/Placemark[IDX]/Point/coordinates												Lon/Lat/Ele-Triples oder Lon/Lat-Paare (z.B. space-separated "12.6433829404414,47.7585958968848")
			/kml/Document/Folder/Placemark[IDX]/ExtendedData/SchemaData/SimpleData/@name[ele]		Höhe
			/kml/Document/Folder/Placemark[IDX]/ExtendedData/SchemaData/SimpleData/@name[time]		Zeitpunkt
			/kml/Document/Folder/Placemark[IDX]/ExtendedData/SchemaData/SimpleData/@name[desc]
			/kml/Document/Folder/Placemark[IDX]/ExtendedData/SchemaData/SimpleData/@name[sym]		Symbolname

         auch geschachtelt (!) mit weiteren Document/Folder

         auch möglich sowohl unter Document als auch Folder:
			Placemark[IDX]/name
			Placemark[IDX]/Point/coordinates												Lon/Lat/Ele-Triples oder Lon/Lat-Paare (z.B. space-separated "12.6433829404414,47.7585958968848")
...
<Folder>
	...
	<Document id="root_doc">
		...
		<Placemark id="default_starting_location">
			...
			<LookAt>
				<longitude>12.37663924336079</longitude>
				<latitude>51.32304038505394</latitude>
				<altitude>0</altitude>
				<heading>5.847877355135048</heading>
				<tilt>0.4142273930023595</tilt>
				<range>49935.001217627</range>
				<gx:altitudeMode>relativeToSeaFloor</gx:altitudeMode>
			</LookAt>
			<styleUrl>#m_ylw-pushpin</styleUrl>
			<Point>
				<coordinates>12.37663924336079,51.32304038505395,0</coordinates>
			</Point>
		</Placemark>

			<Placemark>
				<name>Ortsmarkierung ohne Namen</name>
				<LookAt>
					<longitude>12.75123563186858</longitude>
					<latitude>51.14089294681192</latitude>
					<altitude>0</altitude>
					<heading>2.699772017066245e-08</heading>
					<tilt>45.00000009192021</tilt>
					<range>21804.45251114166</range>
					<gx:altitudeMode>relativeToSeaFloor</gx:altitudeMode>
				</LookAt>
				<styleUrl>#m_ylw-pushpin0</styleUrl>
				<Point>
					<gx:drawOrder>1</gx:drawOrder>
					<coordinates>12.76650743418301,51.12514211070089,0</coordinates>
				</Point>
			</Placemark>
   	...
   	<?xml version="1.0" encoding="UTF-8"?>
		<kml xmlns="http://www.opengis.net/kml/2.2" xmlns:gx="http://www.google.com/kml/ext/2.2" xmlns:kml="http://www.opengis.net/kml/2.2" xmlns:atom="http://www.w3.org/2005/Atom">
		<Document>
			<name>Lutherweg Bayern</name>

			<StyleMap id="trackStyle220">
				<Pair>
					<key>normal</key>
					<styleUrl>#trackStyle580</styleUrl>
				</Pair>
				<Pair>
					<key>highlight</key>
					<styleUrl>#trackStyle360</styleUrl>
				</Pair>
			</StyleMap>

			<Style id="trackStyle360">
				<LineStyle>
					<color>ff073ee7</color>
					<width>3</width>
				</LineStyle>
			</Style>
		...
		...
			...
			<Placemark>
				<name>Lutherweg Bayern</name>
				<open>1</open>
				<description>teilw. Südschlaufe Lutherweg Thüringen</description>
				<styleUrl>#trackStyle220</styleUrl>
				<LineString>
					<coordinates>
						10.90831029,50.38719123999999,510.69531 10.90858278,50.38697146,514.33593 ...
					</coordinates>
				</LineString>
			</Placemark>

			<Placemark>
				<name>Lutherweg Thüringen</name>
				<open>1</open>
				<description>Stich Callenberg CO</description>
				<styleUrl>#trackStyle290</styleUrl>
				<LineString>
					<coordinates>
						10.92291064,50.28022457,337.91796 10.92264418,50.27990656,339.44531 ...
					</coordinates>
				</LineString>
			</Placemark>
