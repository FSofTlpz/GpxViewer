...
namespace GpxViewer {
   public partial class ReadOnlyGpxControl : UserControl {
      ...
      internal class TNode : BaseTreenode {
         ...
         /// </summary>
         public GpxData? Gpx { get; private set; }
         ...
         string _visualText = string.Empty;
         ...
         /// </summary>
         public void VisualNode_Expand() => VisualNode?.Expand();

         /// <summary>
         /// klappt den visuellen Childnode zusammen
         /// </summary>
         public void VisualNode_Collapse() => VisualNode?.Collapse();
      ...
      internal class Loader4GpxFile {
         ...
         ReadOnlyGpxControl ctrl;


         public Loader4GpxFile(ReadOnlyGpxControl ctrl) {
            this.ctrl = ctrl;
         }
         ...
         void loadWorker(ConcurrentBag<TNode> treeNodesSample, ConcurrentBag<TNode> changedTreeNodes, ConcurrentQueue<string> infos) {
            while (treeNodesSample.TryTake(out TNode? tn)) {
            ...
            if (ctrl.InvokeRequired) {
               ...
               ctrl.Invoke(d, [changedTreeNodes]);
            } else
         ...
         TNode? loadDataIfGpxFile(TNode tn, ConcurrentQueue<string> infos) {
            ...
            if (tn.Nodetype == TNode.NodeType.GpxFilename &&
                tn.GpxFilename != null) { // bisher nur Dateiname vorhanden -> Daten einlesen
      ...
      public class ChooseEventArgs {
         ...
         public ChooseEventArgs(GpxData? gpx, Track? track, string? name) {
            Gpx = gpx;
            Track = track;
            Name = name;
         }
      ...
      public class ShowMarkerEventArgs {
         ...
         public ShowMarkerEventArgs(GpxData gpx, bool on) {
            Gpx = gpx;
            On = on;
         }
      }
      ...
      private void treeView1_DrawNode(object sender, DrawTreeNodeEventArgs e) {
         ...
         TNode? tn = e.Node != null ? TNode.GetTN(e.Node) : null;
         ...
         if (tn != null) {
            ...
            switch (tn.Nodetype) {
               ...
               case TNode.NodeType.Track:
                  if (!bTreeViewSelfChecked) { // "echte" Auswahl?
                     Track? track = tn.Track;
                     if (track != null) {
                        ShowTrackEvent?.Invoke(this, new ShowTrackEventArgs(track, tn.VisualNode_IsChecked));

                        if (isSameTrackVisibilityInContainer(track) &&
                            tn.Parent != null &&
      ...
      void visualTreeAfterSelect(TNode? tn) {
         if (tn != null &&
            ...
            switch (tn.Nodetype) {
               ...
               default:
                  SelectTrackEvent?.Invoke(this, new ChooseEventArgs(null, null, null));
                  break;
            }
         }
      }
      ...
      /// <returns></returns>
      void replaceDummynodeWithTracknodes(TNode tn, GpxData gpx, bool externname) {
      ...
      /// </summary>
      private delegate void SafeCallDelegate4TreeNodePoorGpxAllExtBool2Void(TNode tn, GpxData gpx, bool externname);
      ...
      /// <returns></returns>
      static GpxData buildPoorGpxAllExt(string gpxfilename, string? gpxpicturefilename, string? picturereferencepath, int stdcolor) {
         Color col;
         switch (stdcolor) {
            case 1:
         ...
         List<Color> trackcolors = gpx.Load(gpxfilename, true);      // GPX-Datei einlesen
         gpx.GpxFilename = gpxfilename;
         gpx.GpxPictureFilename = gpxpicturefilename;
         gpx.TrackColor = col;
         gpx.TrackWidth = (float)VisualTrack.StandardWidth;

         // für alle Tracks die Standardfarbe des Containers setzen (hat Vorrang vor individueller Farbe!)
         if (gpx.TrackColor.ToArgb() != Color.Empty.ToArgb())
            for (int i = 0; i < gpx.TrackList.Count; i++)
               gpx.TrackList[i].LineColor = gpx.TrackColor;
         else
            for (int i = 0; i < gpx.TrackList.Count; i++)
               gpx.TrackList[i].LineColor = trackcolors[i];

         //for (int i = 0; i < gpx.TrackList.Count; i++)
         //   gpx.TrackList[i].LineColor = trackcolors[i] == Color.Empty ?
         //            gpx.TrackColor :
         //            trackcolors[i];
         ...
         if (!string.IsNullOrEmpty(gpxpicturefilename)) {
            ...
            for (int i = 0; i < pictgpx.Waypoints.Count; i++) {
               Gpx.GpxWaypoint wp = pictgpx.Waypoints[i];
               ...
               wp.Name = Path.GetFullPath(Path.Combine(referencepath != null ? referencepath : string.Empty, wp.Name)); // GetFullPath() -> entfernt unnötige "." und ".."
      ...
      TNode addDelimiterTreeNode(TNode parent, string txt, TNode? tnnext = null) {
         ...
         int idx = parent != null && tnnext != null ?
                     parent.ChildNodes.IndexOf(tnnext) :
                     -1;
         parent.InsertChild(idx, tn); // idx < 0 => Append
            ...
            if (tn.VisualNode_IsChecked) {
               if (tn != null)
                  switch (tn.Nodetype) {
                     ...
                     case TNode.NodeType.Track:
                        if (tn.Track != null && !tracklst.Contains(tn.Track))
      ...
      /// <param name="tnlst"></param>
      /// <returns></returns>
      TNode? getNode4Gpx(GpxData gpx, IList<TNode> tnlst) {
         TNode? result = null;
         foreach (TNode tn in tnlst)
            BaseTreenode.Walk(tn,
                              (BaseTreenode btn) => {
      ...
      /// <summary>
      /// liefert die Liste aller <see cref="GpxData"/> der Subnodes des TreeNode
      /// </summary>
      /// <param name="parent"></param>
      /// <returns></returns>
      List<GpxData> getAllGpxContainerFromSubnodes(TNode parent) {
         List<GpxData> lst = new List<GpxData>();
         ...
         foreach (TNode tn in parent.ChildNodes) {
            switch (tn.Nodetype) {
               ...
               case TNode.NodeType.GpxObject:
                  if (tn.Gpx != null && !lst.Contains(tn.Gpx))
                     lst.Add(tn.Gpx);
                  break;
            }
         }
      ...
      public void AddFile(string filename) {
         ...
         if (ext == ".gpx" ||
            ...
            TNode? tn = addGpxfileTreeNode(root,
                                  filename,
                                  null,
                                  null,
                                  null,
         ...
         foreach (TNode tn in root.ChildNodes) {
            BaseTreenode.Walk(tn,
                              ...
                              (BaseTreenode btn, List<Marker> result) => {
                                 TNode? tnode = btn as TNode;
                                 if (tnode != null &&
                                     tnode.Nodetype == TNode.NodeType.GpxObject) {
      ...
      /// <param name="gpx"></param>
      /// <param name="visible"></param>
      public void ShowGpx(GpxData gpx, bool visible) {
      ...
      /// <param name="gpx"></param>
      /// <returns></returns>
      public bool GpxIsVisible(GpxData gpx) {
         TNode? tn = getNode4Gpx(gpx, root.ChildNodes);
         return tn != null ?
                        tn.VisualNode_IsChecked :
                        false;
      ...
      /// <summary>
      /// liefert die akt. ausgewählte <see cref="GpxData"/> oder den <see cref="Track"/>
      /// </summary>
      /// <param name="gpx"></param>
      /// <param name="track"></param>
      /// <returns>false, wenn nicht (sinnvolles) ausgewählt ist</returns>
      public bool GetSelectedObject(out GpxData? gpx, out Track? track) {
         track = null;
         gpx = null;
      ...
      /// </summary>
      /// <returns></returns>
      public List<GpxData> GetAllSubGpxContainerFromSelected() {
      ...
      private void contextMenuStripInfo_Opening(object sender, System.ComponentModel.CancelEventArgs e) {
