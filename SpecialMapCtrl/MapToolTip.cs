...
namespace SpecialMapCtrl {
   ...
   public class MapToolTip : IDisposable {
      ...
      public MapToolTip(MapMarker marker) {
         Marker = marker;
...
#else
                  g.DrawString(String.Format("{0}", vec2[0]), TitleFont, Foreground, rectText.Location, Format);
                  rectText.X += st.Width + 2;
                  if (vec2.Length > 1)
                     g.DrawString(vec2[1], Font, Foreground, rectText.Location, Format);
#endif
                  rectText.X -= st.Width + 2;
